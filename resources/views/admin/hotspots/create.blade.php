...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.hotspots.store') }}" method="POST">
            @csrf
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <label for="pitch" class="form-label">Pitch (vertical angle, -90 to 90)</label>
                    <input type="number" class="form-control @error('pitch') is-invalid @enderror" id="pitch" name="pitch" step="0.1" value="0" required>
                    <small class="text-muted">0 = horizontal, -90 = down, 90 = up</small>
                    @error('pitch') <span class="invalid-feedback">{{ $message }}</span> @enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label for="yaw" class="form-label">Yaw (horizontal angle, 0-360)</label>
                    <input type="number" class="form-control @error('yaw') is-invalid @enderror" id="yaw" name="yaw" step="0.1" value="0" required>
                    <small class="text-muted">0 = North, 90 = East, 180 = South, 270 = West</small>
